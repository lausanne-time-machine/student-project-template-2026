...
# Lausanne, Cadastre Rénové (1888)
Cette page présente les données géoréférencées extraites du cadastre rénové de Lausanne (1888), avec la possibilité de filtrer les entrées du registre associé et de visualiser les points correspondants sur une carte interactive.
Une couche "heatmap" est également disponible pour visualiser la densité de tous les points.
...
function createMapAndLayer(mapContainer, geojsonData) {
    ...
    // Historical tile layer (City Map 1890) — added as an overlay so it sits beneath the GeoJSON points and heatmap in the layer control
    const lebetLayer = L.tileLayer(
        "https://geo-timemachine.epfl.ch/geoserver/gwc/service/wmts/rest/TimeMachine:lausanne-1890-lebet/{style}/{TileMatrixSet}/{TileMatrixSet}:{z}/{y}/{x}?format=image/png",
        {
            style: "raster",
    ...
    layerControl.addOverlay(lebetLayer, "Carte Lebet (1890)");
